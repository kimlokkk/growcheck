...
bool _useDesktopSensoryResult2Layout(BuildContext context) {
...
class _SensoryProfileResult2State extends State<SensoryProfileResult2> {
  ...
  Future<void> fetchResultData() async {
    try {
      final response = await http.post(
        Uri.parse(ApiConfig.parentsFlutter('get_sensory_result_2.php')),
        /*Uri.parse(
            'http://app-kizzu.test/growkids/flutter_growcheck_parents/get_sensory_result_2.php'),*/
  ...
  Map<String, List<Map<String, dynamic>>> groupByCategory() {
    ...
    for (final q in answers) {
      final cat =
          (q['category'] ?? q['category_name'] ?? 'Uncategorized').toString();
    ...
    final leftovers = grouped.keys.toList()
        ...
        children: [
          CircleAvatar(
            ...
            child: Icon(Icons.psychology_rounded,
                color: Growkids.purple, size: 3.2.h),
          ),
          SizedBox(width: 3.w),
          Expanded(
            child: Column(
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    if (totalBand.trim().isNotEmpty)
                      _pill(
                        text: totalRange.trim().isNotEmpty
                            ? '$totalBand ($totalRange)'
                            : totalBand,
                        bg: bandColor.withValues(alpha: 0.20),
                        fg: Colors.white,
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          if (quad.trim().isNotEmpty &&
                              quad.trim().toLowerCase() != 'no quadrant')
                            _pill(
                              text: quad,
  ...
  Widget build(BuildContext context) {
    if (_useDesktopSensoryResult2Layout(context)) {
      ...
      body: isLoading
          ...
          : errorMsg != null
              ...
              : SensoryResultDesktop(
                  ...
                  totalScore: _safeStr(
                    resultData['total_score'] ?? _resultsMap['total_score'],
                    '0',
                  ),
                  totalBand: _safeStr(
                    resultData['total_band'] ?? _resultsMap['total_band'],
                    '',
                  ),
                  quadrantSummary: const {},
                  categorySummary: _categorySummary,
                  groupedAnswers: groupByCategory(),
                ),
    );
  }
