...
import 'package:growcheck_app_v2/pages/ssp/sensory_result_desktop.dart';
import 'package:growcheck_app_v2/ui/colour.dart';
import 'package:http/http.dart' as http;
import 'package:intl/intl.dart';
import 'package:sizer/sizer.dart';

bool _useDesktopSensoryResultLayout(BuildContext context) {
...
class SensoryProfileResult extends StatefulWidget {
  ...
  const SensoryProfileResult(
      {super.key, required this.assessmentId, required this.studName});
...
class _SensoryProfileResultState extends State<SensoryProfileResult> {
  ...
  Future<void> fetchResultData() async {
    try {
      final response = await http.post(
        Uri.parse(ApiConfig.parentsFlutter('get_sensory_result.php')),
        /*Uri.parse(
            'http://app-kizzu.test/growkids/flutter_growcheck_parents/get_sensory_result.php'),*/
      ...
      if (response.statusCode == 200) {
        ...
        final ok =
            (normalized['status']?.toString().toLowerCase() == 'success') ||
        ...
        if (!ok) {
          setState(() {
            errorMsg =
                normalized?['message']?.toString() ?? 'Unable to load result.';
            isLoading = false;
          });
          return;
        }
  ...
  Map<String, List<Map<String, dynamic>>> _groupAnswersByCategory(
    ...
    for (final a in answers) {
      final cat =
          _readStr(a, ['category', 'category_name', 'cat', 'section']).trim();
    ...
    final leftover = grouped.keys.toList()
      ..sort((a, b) => a.toLowerCase().compareTo(b.toLowerCase()));
  ...
  Color _badgeBg(bool needs) => needs
  ...
  Widget _heroHeader() {
    ...
    final totalScore =
        _safeStr(resultData['total_score'] ?? results['total_score'], '0');
    final totalBand =
        _safeStr(resultData['total_band'] ?? results['total_band'], '');
    final totalRange = _safeStr(
        resultData['total_band_range'] ?? results['total_band_range'], '');
        ...
        children: [
          CircleAvatar(
            ...
            child: Icon(Icons.psychology_rounded,
                color: Growkids.purple, size: 4.h),
          ),
          SizedBox(width: 3.w),
          Expanded(
            child: Column(
          ...
          ...entries.map((e) {
            ...
            return Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(18),
                border:
                    Border.all(color: _badgeFg(needs).withValues(alpha: 0.25)),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      _pill(
                        text: 'Score: $total',
                        bg: Growkids.purple.withValues(alpha: 0.10),
                        fg: Growkids.purple,
          ...
          ...entries.map((e) {
            ...
            return Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(18),
                border:
                    Border.all(color: _badgeFg(needs).withValues(alpha: 0.20)),
              ),
              child: Row(
                children: [
                  Expanded(
          ...
          return Container(
            ...
            child: ExpansionTile(
              ...
              children: items.map((q) {
                final qText =
                    _readStr(q, ['question_text', 'question', 'text', 'q'])
                        .trim();
                final score = _readNum(q, [
                      'value',
                      'score',
                      'points',
                      'selected',
                    ...
                    0;
                final quadRaw =
                    _readStr(q, ['quadrant', 'quadrant_name', 'quad']).trim();
  ...
  Widget build(BuildContext context) {
    if (_useDesktopSensoryResultLayout(context)) {
  ...
  Widget _buildDesktop() {
    ...
    return Scaffold(
      ...
      body: isLoading
          ...
          : errorMsg != null
              ? _desktopError()
              : SensoryResultDesktop(
                  studentName: widget.studName,
                  assessmentDate:
                      _formatDateSafe(resultData['assessment_date']),
                  age: _safeStr(resultData['age']),
