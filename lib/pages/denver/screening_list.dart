...
class _ScreeningListState extends State<ScreeningList> {
  ...
  Future<void> fetchData() async {
    final response = await http
        .post(Uri.parse(ApiConfig.flutter('screening_list_v2.php')), body: {
      /*await http.post(
            Uri.parse(
                'http://app-kizzu.test/growkids/flutter/screening_list_v2.php'),
            body: {*/
      "staff_id": id,
    });
                                    ...
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (BuildContext context) =>
                                            ScreeningResult(
                                          screeningDate: displayedData[index]
                                              ['screening_date'],
                                          studentId: displayedData[index]
                                              ['student_id'],
                                          screeningId: displayedData[index]
                                              ['screening_id'],
                                          age: double.parse(
                                              displayedData[index]['age']),
                                          studentName: displayedData[index]
                                              ['stud_name'],
                                          ageFineMotor: double.parse(
                                              displayedData[index]
                                                  ['age_fine_motor']),
                                          ageGrossMotor: double.parse(
                                              displayedData[index]
                                                  ['age_gross_motor']),
                                          ageLanguage: double.parse(
                                              displayedData[index]
                                                  ['age_language']),
                                          agePersonal: double.parse(
                                              displayedData[index]
                                                  ['age_personal_social']),
